...
class Paginator:
    ...
    vote_list_id: int = attrs.field(repr=False, kw_only=True, default=None)
    """The id of the votelist this paginator is associated with"""
    vote_list_item_ids: List[int] = attrs.field(repr=False, kw_only=True, default=None)
    """The ids of the movies in the votelist this paginator is associated with"""
    page_ranking: List[Optional[int]] = attrs.field(init=False)
    ...
    ranking_callback: Callable[..., Coroutine] = attrs.field(repr=False, default=None)
    """A coroutine to call should the ranking select menu be used"""
    ...
    show_ranking_menu: bool = attrs.field(repr=False, default=False)
    """Should a select menu be shown for ranking"""
        ...
        self.client.add_component_callback(
            ComponentCommand(
                ...
                listeners=[
                    ...
                    f"{self._uuid}|next",
                    f"{self._uuid}|last",
                    f"{self._uuid}|ranking"
                ],
            )
        )

        if self.show_ranking_menu:
            self.page_ranking = [None] * len(self.pages)
    ...
    @classmethod
    def create_from_movie_list(cls,
                               client: "Client",
                               *embeds: Embed,
                               vote_list_id: int,
                               vote_list_movie_ids: List,
                               ranking_callback: Callable[..., Coroutine],
                               finished_callback: Callable[..., Coroutine]) -> "Paginator":
        """
        Create a paginator system from a list of Movie embeds.

        Args
            client: A reference to the client
            *embeds: The embeds to use for each page
            movie_list_id: The id of the movie list
            movie_list_item_ids: The ids of the movie list items
        """

        return cls(client,
                   pages=list(embeds),
                   vote_list_id=vote_list_id,
                   vote_list_item_ids=vote_list_movie_ids,
                   show_ranking_menu=True,
    ...
    def create_components(self, disable: bool = False) -> List[ActionRow]:
        ...
        if self.show_ranking_menu:
            output.append(
                StringSelectMenu(
                    *[StringSelectOption(default=True if ((num+1) == self.page_ranking[self.page_index]) else False,
                                         label=f"Rank {str(num + 1)}",
                                         value=str(num + 1)) for num in range(len(self.pages))],
                    custom_id=f"{self._uuid}|ranking",
                    min_values=1,
                    max_values=1,
                    disabled=disable
                )
    ...
    async def send(self, ctx: BaseContext, ephemeral: bool = False) -> Message:
        """
        Send this paginator.

        Args:
            ctx: The context to send this paginator with
            ephemeral: Should the message be ephemeral?
    ...
    async def _on_button(self, ctx: ComponentContext, *args, **kwargs) -> Optional[Message]:
        ...
        match ctx.custom_id.split("|")[1]:
            ...
            case "ranking":
                ranking = ctx.values[0]
                self.page_ranking[self.page_index] = int(ranking)
                if self.ranking_callback:
                    vote_list_item_id = self.vote_list_item_ids[self.page_index]

                    return await self.ranking_callback(ctx,
                                                       vote_list_id=self.vote_list_id,
                                                       vote_list_item_id=vote_list_item_id,
                                                       ranking=ranking
                                                       )
            case "callback":
                if self.callback:
                    return await self.callback(ctx, self.page_ranking)
